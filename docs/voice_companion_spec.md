...
文字チャットでもAIが返答するならコイン消費対象。固定文言・システム都合の文言・エラー文言は消費しない。
...
開始前に最低1応答分のコイン残高を確認する。通話全体の前払いはさせない。
...
固定文言はAI応答ではなくアプリ側の状態文言であり、コイン消費しない。固定文言にTTSを使っても、AI応答でなければ消費しない。

コイン消費は、キャラ音声が1回再生開始した時点で1応答分。STTだけ、LLM生成だけ、TTS失敗、再生失敗、無言、聞き取り失敗、固定文言では消費しない。
...
Talkで疑似電話へ進み、キャラ音声が再生開始したら通常ルールで消費。
...
- ユーザー入力に対してAIが生成した応答がキャラ音声として再生開始された時
- AI猫がユーザー入力に対して感情分類して鳴き声反応を返す時
- 文字チャットでAIが返答する時

固定文言はLLMを呼ばないので消費しない。固定文言にTTSを使っても、AI応答でなければ消費しない。TTS生成失敗・再生失敗は消費しない。
...
- source_message_id / source_call_turn_id
- consumed_amount
- triggered_by: tts_audio_started / ai_cat_response_selected
...
課金・誤課金防止のため、音声再生開始時刻や消費IDは保存する。
