# VoiceCompanion 仕様書 v4.6

**版数: v4.6 ／ 最終更新日: 2026-07-18**

変更履歴:

- 2026-07-18 v4.6: 疑似電話のTTSをクラウドTTS(Aivis)からオンデバイスTTS(ONNX Runtime + Style-Bert-VITS2系AIVMX)へ変更。TTS従量課金では採算が成立しないため。A8-3を新設し、方式・モデル構成(TTS本体/DeBERTa BERTともに動的INT8)・文末単位ストリーミング再生・声モデル配信(初期同梱せずキャラ選択時にCloudflare R2からDL)・実測基準値・本実装のフリーズ区間ログ要件を定義。H1に声モデル利用条件、H4にオンデバイスTTS採用、F6に低速端末対策の未決項目を反映。
...
- クラウドTTS(Aivis)ではなく、端末上のONNX Runtime + Style-Bert-VITS2系AIVMX(桜音モデル)によるオンデバイスTTSを採用する。
...
AivisHub上で商用利用可能なACML 1.0モデルを確認済み。アプリへの組み込み・再配布が可能であることを確認済み(桜音モデル)。
...
オンデバイスTTSの本体モデルには桜音(AIVMX)を継続採用する。方式・量子化・配信はA8-3を正とする。
...
その後、TTSはオンデバイス化を採用決定(A8-3)。オンデバイスTTSの実測(POCO X6 Pro / CPU EP 4スレッド)は、1文目の再生開始まで約0.9〜1.0秒(文末単位ストリーミング)、INT8化による音質劣化なし。以後のTTFA前提はオンデバイスTTSを基準にする。
