# VoiceCompanion 仕様書 v4.8

**版数: v4.8 ／ 最終更新日: 2026-07-20**
...
- クラウドTTS(Aivis)ではなく、端末上のONNX Runtime + Style-Bert-VITS2系AIVMX(各キャラの声モデル)によるオンデバイスTTSを採用する。対象はv1の6キャラ全員(H1参照)。桜音1モデルに絞る前提にはしない。
...
- 猫の鳴き声素材
- 固定音声の具体作成
- 低速端末対策: オンデバイスTTSが基準端末水準に満たない端末で実用速度に達しない場合の方針を実装時に決める。端末条件による配信制限とするか、クラウドTTSフォールバックを用意するかを含む(A8-3の実測基準参照)
...
Android実機で STT → LLM → Aivis TTS(クラウド) → 音声再生まで成立。TTFAは約2秒前後で確認。

その後、TTSはオンデバイス化を採用決定(A8-3)。オンデバイスTTSの実測(POCO X6 Pro / CPU EP 4スレッド、桜音を基準モデルとして計測)は、1文目の再生開始まで約0.9〜1.0秒(文末単位ストリーミング)、INT8化による音質劣化なし。以後のTTFA前提はオンデバイスTTSを基準にする。
