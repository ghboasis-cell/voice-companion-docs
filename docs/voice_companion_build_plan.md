...
**版数: v5.346 ／ 最終更新日: 2026-09-25**

（v5.346: **記憶の作業（2026-09-24〜25）のまとめ。main へ反映済み（PR #109、`b58f9d4`）。production は未反映**（9/25、利用者が決めた）。
- 経緯（v5.333〜v5.345 の13件：止血・Phase 2〜6・抽出精度、途中の直しと staging の確認）は、中身を変えずに
  `docs/memory_work_log_20260924.md` へ移した。
- **最終の状態**（spec v5.134 D12・D28・E4・E5）：
  - 夜間の記憶処理は人間のキャラと、ユーザーが話した日だけ。生の履歴は7日まで。
  - 会話（トーク・通話・通知・自発着信）・告白・夜間処理が読む長期記憶は、構造化記憶 `memory_statements` の確定だけ（新しい順に最大50件）。
    旧方式（`memory_facts`・長期記憶JSON・`character_memory_facts`）は読まない・新しく書かない。表とデータは残す。
  - fact_key はAIに作らせない。既にある記憶を e番号・同じ抽出を n番号で見せ、AIは same_as で同じ事実軸を選ぶ。新しい事実軸は `k.<UUID>`。
  - 置き換え：好み・プロフィール・関係・安全は、同じ事実軸で本人があとから言った方にする。予定・出来事・物語は確定どうしでは置き換えず、
    否定・否定のあとの確定は、AIが指した既にある1行だけを同じ日付のときに置き換える。問いかけ・推測・候補は確定・否定が入れば置き換える。
  - 抽出：否定の content は否定された元の事実、予定・出来事の並べた対象は別々の行、引用は15文字以内の原文の抜き出し（発言に無ければ捨てる）。
    日付は抜き出しで決め、決まらなければ予定・出来事・物語だけ発言全体から。問いかけと分類の直しは抜き出しを含む一文で見る。
  - モデル設定は `qwen3.8-flash`・考える処理なし・`json_object` のまま（考える処理は1回90秒超、json_schema は改善なし、と実測）。
- **反映の状況**：
  - コード：`feature/call-screen` の最終コミット `6d41251` まで main へ入った（`b58f9d4`）。
  - staging：migration `20260924120000`〜`20260924200000` の10本を反映済み。memory-worker は版111。ほかの関数は、9/25 00:16 に
    関数名なしの deploy で全関数が出し直された版（経緯ファイルの v5.345 に記録。コードは `570c5c9` で、記憶まわり以外は main と同じ）。
  - production：未反映。
- **本番へ出すとき**：DB の migration 10本（`20260924120000`〜`20260924200000`）→ memory-worker → chat-reply → voice-turn →
  daily-notification-worker → spontaneous-call-dispatch-worker。**関数は名前を付けて1つずつ出す**（関数名なしの `supabase functions deploy` は
  全関数を出す。`!` で頼むときは `reports/dw.sh` のように名前を中に固定した短いスクリプトにする）。
- staging にだけあるもの：調査用関数 `memory-extraction-probe`（未コミット。DBは読まない・書かない。抽出精度の比較に使った）。
- staging に残っている古い行（巻き添えで superseded になったカレーの確定2行、ゼリーの確定と否定の両方、最初の29件の不揃いな fact_key）は
  直さない（既にある行は手で直さない、と決めたとおり）。
- **残した課題**：
  1. same_as が、似た過去の事実軸を選びたがることがある（指した1行だけを置き換える安全策で、破壊的な害は防げている）。
  2. 明示的な問いかけ（「明日ジム行くの？」）の実通話での確認。
  3. 会話の返事で、キャラの予定をユーザーの予定として話すことがある（記憶とは別件・未調査。9/24 オーディションの曲選び、9/25 莉緒が自分のレッスン中に
     「レッスン頑張ってね」）。記憶では subject=character で正しく入っている。
  4. Phase 5 からの後日確認：新しい日の夜間処理で `context_processed_through` が先へ進むこと、告白イベントの実動。
  5. 記憶とは別の気づき：結衣が「6時半まで仕事」を「深夜まで働いてたの？」と取り違えた（9/24）。
